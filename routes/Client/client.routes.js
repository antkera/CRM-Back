...
const clientValidation = [
  ...
  body('dni').matches(/^[0-9]{8}[A-Z]$/).withMessage('DNI no válido. Debe tener 8 dígitos seguidos de una letra mayúscula.'),
  body('direccion').notEmpty().withMessage('La dirección es requerida'),
  body('telefono').notEmpty().withMessage('El teléfono es requerido'),
  body('email').isEmail().withMessage('Email no válido. Debe seguir el formato usuario@dominio.com'),
...
router.post('/newClient', clientValidation, async (req, res, next) => {
  ...
  const { nombre, apellidos, dni, direccion, telefono, email, hijos } = req.body;
  const userId = req.payload._id;

  try {
    const existingClient = await Client.findOne({ dni });
    if (existingClient) {
      return res.status(400).json({ message: 'Ya existe un cliente con este DNI' }); // Devuelve un error si el DNI ya está en uso
